...
fig, axes = plt.subplots(
    ncols=2, nrows=2, figsize=(12, 12), facecolor='w', sharex=True, sharey=True, subplot_kw=dict(aspect='equal')
)
for param, unit, ax, gdf in zip(param_codes, units, axes.flatten(), gdfs):
    ...
    for source, data in gdf.groupby('source'):
        data.plot(
            ax=ax, marker='.', categorical=True, markersize=4, linewidth=0, legend=True,
...
fig.legend(
    handles, labels, loc='lower center', ncol=5, bbox_to_anchor=[0.5, 0.15], frameon=False, markerscale=8, fontsize=10
)
plt.savefig(
...
fig.tight_layout(w_pad=-4, h_pad=-21)
fig.legend(
    handles, labels, loc='lower center', ncol=5, bbox_to_anchor=[0.5, 0.15], frameon=False, markerscale=6, fontsize=10
)
plt.savefig(
